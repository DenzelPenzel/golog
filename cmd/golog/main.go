...
func main() {
	...
	cmd := &cobra.Command{
		...
		RunE:    cli.run, // set on command when the command runs
	}
...
func setupFlags(cmd *cobra.Command) error {
	...
	dataDir := path.Join(os.TempDir(), "golog")
...
// read configuration and prepares the agent's configuration
...
2) Handling signals from the operating system;
3) Shutting down the agent gracefully when the operating system terminates
the program
*/

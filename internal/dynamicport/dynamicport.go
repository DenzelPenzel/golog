...
func GetWithErr(n int) (ports []int, err error) {
	...
	if n > blockSize-1 {
		return nil, fmt.Errorf("dynamicport: block size is too small for ports requested")
	}
...
func initialize() {
	if lowPort+maxBlocks*blockSize > maxPorts {
		panic("dynamicport: block size is too big or too many blocks requested")
	}
	...
	panic("dynamicport: can't allocated port block")
}
